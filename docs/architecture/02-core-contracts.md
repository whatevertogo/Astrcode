...
- `AgentLoop` 基本清晰，已接入 policy 和 approval
- `Capability` 已经是正确方向，`Tool.capability_descriptor()` 已实现
- `Policy` 已收敛为正式契约，`PolicyEngine` 三态（Allow/Deny/Ask）已落地
- `Event` 已明确区分 `AgentEvent`（运行时观测）和 `StorageEvent`（持久化/replay）

因此，后续的重构重点不在重写 loop，而在补齐 Policy 与 Event 这两个契约。

### Policy Contract 当前实现

- `crates/core/src/policy/engine.rs` — `PolicyEngine` trait 定义三个决策点：
  - `check_model_request()` — turn 开始时检查/改写模型请求
  - `check_capability_call()` — tool call 前判断 Allow/Deny/Ask
  - `decide_context_strategy()` — 上下文压力时决定策略（待接入 token budgeting）
- `AllowAllPolicyEngine` — 默认实现，保证无自定义 policy 时行为兼容
- `crates/runtime/src/agent_loop/turn_runner.rs` — 已接入 `check_model_request()`
- `crates/runtime/src/agent_loop/tool_cycle.rs` — 已接入 `check_capability_call()` 三态分支

### Event Contract 当前实现

- `crates/core/src/event/domain.rs` — `AgentEvent` 面向 SSE 推送和 UI 展示
- `crates/core/src/event/types.rs` — `StorageEvent` 面向 JSONL 持久化和 replay
- `crates/core/src/event/translate.rs` — `EventTranslator` 做 StorageEvent → AgentEvent 投影
- `crates/core/src/event/store.rs` — `EventLog` 实现 append-only JSONL 写入
- 两者通过投影关联，不强制等同，避免瞬时 UI 事件污染持久化模型
