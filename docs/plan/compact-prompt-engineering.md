...
| 改进项 | 状态 | 说明 |
|--------|------|------|
| `<analysis>` + `<summary>` XML 块 | ✅ 已有 | `build_compact_system_prompt()` 要求 LLM 返回两个 XML 块 |
...
| **第三方称语气** | 无约束 | P2 |
| **Prompt 模板外部化** | inline 在 `build_compact_system_prompt()` | P3 |
...
### 3.1 改进后的 `build_compact_system_prompt()`
...
fn build_compact_system_prompt(base_system_prompt: Option<&str>) -> String {
    ...
    if let Some(bsp) = base_system_prompt.filter(|s| !s.trim().is_empty()) {
...
```rust
fn build_incremental_compact_prompt(base_system_prompt: Option<&str>, previous_summary: &str) -> String {
    let mut prompt = build_compact_system_prompt(base_system_prompt);
...
let summary_prompt = if let Some(prev) = previous_summary {
    build_incremental_compact_prompt(base_system_prompt, prev)
} else {
    build_compact_system_prompt(base_system_prompt)
};
```
...
| 优先级 | 内容 | 改动范围 | 原因 |
|--------|------|---------|------|
| **P0** | 替换 `build_compact_system_prompt()` 为改进版 | `compaction.rs` 约 30 行 | 收益最大：优先级 + NO_TOOLS 位置 + Output ONLY |
...
| **P3** | Hook 扩展点：`override_system_prompt` 注入自定义 compact prompt | 已由 `ModifyCompactContext` 支持，无需额外代码 | 已完成 |
...
4. **Hook 覆盖**：`PreCompact` hook 的 `override_system_prompt` 已经可以完全覆盖 compact prompt。改进后的 prompt 是默认实现，插件可以替换。
