...
| 设计要素 | 实现位置 |
|---------|---------|
| 六个协作工具 | `crates/runtime-agent-tool/src/{spawn,send,wait,close,resume,deliver}_tool.rs` |
| `AgentInboxEnvelope` 投递层 | `crates/runtime-agent-control/src/lib.rs`（push_inbox / wait_for_inbox） |
| Parent Reactivation | `crates/runtime/src/service/execution/mod.rs`（reactivate_parent_agent_if_idle） |
| `CapabilityRouter` 唯一入口 | `crates/runtime-registry/src/router.rs`，协作工具通过 CapabilityRouter 注册 |
| `ToolRegistry` 退化为测试辅助 | `crates/runtime-registry/src/tool.rs`，仅在测试和 bootstrap 装配中使用 |
| Leaf-first cascade close | `crates/runtime/src/service/execution/cancel.rs`（close_agent_subtree） |
| Direct-parent-only delivery | `crates/runtime/src/service/execution/subagent.rs`（deliver_to_parent） |
| 协作 observability | `crates/runtime-execution/src/lib.rs`（collaboration_action_label + child_delivery_outcome_label） |
