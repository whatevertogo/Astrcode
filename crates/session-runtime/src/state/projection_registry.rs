...
use astrcode_core::{
    ...
    TaskSnapshot, TurnProjectionSnapshot, event::PhaseTracker,
};
...
#[derive(Debug, Clone, Default)]
pub(super) struct ChildNodeProjection {
...
impl ChildNodeProjection {
    ...
    pub(super) fn upsert(&mut self, node: ChildSessionNode) {
        self.nodes.insert(node.sub_run_id().to_string(), node);
    }
...
impl ActiveTaskProjection {
    ...
    #[cfg(test)]
    fn replace(&mut self, snapshot: TaskSnapshot) {
        apply_snapshot_to_map(&mut self.snapshots, snapshot);
    }
...
pub(crate) struct ProjectionRegistry {
    ...
    pub(super) children: ChildNodeProjection,
