//! # 文件访问跟踪
//!
//! compaction 之后，仅靠摘要往往不够让模型继续安全地编辑代码。
//! 这里保留最近 `readFile` 的访问轨迹，并在压缩后回补一小段最新文件内容。
...
mod tests {
    ...
    #[test]
    fn tracker_recovers_recent_files_in_reverse_access_order() {
        let tempdir = tempdir().expect("tempdir should exist");
        let working_dir = tempdir.path();
        fs::write(working_dir.join("a.rs"), "fn a() {}\n").expect("a.rs should write");
        fs::write(working_dir.join("b.rs"), "fn b() {}\n").expect("b.rs should write");
        ...
        let tracker = FileAccessTracker::seed_from_messages(&messages, 8, working_dir);
        let recovered = tracker.build_recovery_messages(FileRecoveryConfig {
            max_tracked_files: 8,
            max_recovered_files: 2,
            recovery_token_budget: 400,
        });

        assert_eq!(recovered.len(), 2);
        assert!(matches!(
            &recovered[0],
            LlmMessage::User { content, .. } if content.contains("a.rs")
        ));
        assert!(matches!(
            &recovered[1],
            LlmMessage::User { content, .. } if content.contains("b.rs")
        ));
    }

    #[test]
    fn tracker_uses_metadata_path_when_tool_returns_absolute_location() {
        let tempdir = tempdir().expect("tempdir should exist");
        let working_dir = tempdir.path();
        let absolute = working_dir.join("nested").join("file.rs");
        fs::create_dir_all(absolute.parent().expect("parent should exist"))
            .expect("parent dir should exist");
        fs::write(&absolute, "fn nested() {}\n").expect("file should write");
        ...
        tracker.record_tool_result(&tool_call, &result, working_dir);
        let recovered = tracker.build_recovery_messages(FileRecoveryConfig {
            max_tracked_files: 4,
            max_recovered_files: 1,
            recovery_token_budget: 200,
        });

        assert_eq!(recovered.len(), 1);
        assert!(matches!(
            &recovered[0],
            LlmMessage::User { content, .. } if content.contains("nested")
        ));
    }
}
