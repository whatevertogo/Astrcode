...
#[cfg(test)]
pub(super) fn merge_compact_prompt_context(
...
pub(super) fn is_prompt_too_long(error: &astrcode_kernel::KernelError) -> bool {
    let message = error.to_string();
    xml_parsing_impl::contains_ascii_case_insensitive(&message, "prompt too long")
        || xml_parsing_impl::contains_ascii_case_insensitive(&message, "context length")
        || xml_parsing_impl::contains_ascii_case_insensitive(&message, "maximum context")
        || xml_parsing_impl::contains_ascii_case_insensitive(&message, "too many tokens")
}
