...
use astrcode_core::{CapabilityInvoker, PromptDeclaration};
use log::{info, warn};
use serde::Serialize;
...
pub(crate) fn build_server_invokers(
    ...
    for tool in tools {
        ...
        let capability_name = bridge.capability_name().to_string();
        if registered.insert(capability_name.clone()) {
            invokers.push(Arc::new(bridge));
        } else {
            warn!(
                "MCP server '{}' skipped duplicate capability '{}'",
                server_name, capability_name
            );
        }
    }
    ...
    for prompt in prompts {
        ...
        if registered.insert(capability_name.clone()) {
            ...
            warn!(
                ...
                server_name, prompt.name, capability_name
            );
        }
    }
