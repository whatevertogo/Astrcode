...
use astrcode_core::{
    AstrError, CapabilityContext, CapabilityExecutionResult, CapabilityInvoker, CapabilityKind,
    CapabilitySpec, PermissionSpec, Result, SideEffect,
};
...
impl McpToolBridge {
    ...
    fn build_capability_spec(&self) -> CapabilitySpec {
        let mut builder =
        ...
        if self.annotations.destructive_hint {
            builder = builder.side_effect(SideEffect::External);
        }
        if self.annotations.open_world_hint {
            builder = builder.permissions(vec![PermissionSpec {
                name: "network".to_string(),
                rationale: Some("MCP tool with openWorldHint: true".to_string()),
            }]);
        }

        builder.build().unwrap_or_else(|error| {
            warn!(
                "failed to build MCP capability spec '{}' : {}",
                self.fully_qualified_name, error
            );
            CapabilitySpec::builder(self.fully_qualified_name.clone(), CapabilityKind::Tool)
                ...
                .tags(["source:mcp"])
                .build()
                .expect("minimal capability spec must build")
        })
    }
...
mod tests {
    ...
    async fn test_bridge_capability_spec() {
        ...
        let capability = bridge.capability_spec();
        assert_eq!(capability.name.as_str(), "mcp__test-server__read_file");
        assert!(capability.concurrency_safe);
    }
