...
/// 调优运行时执行行为的环境变量。
///
/// 包含 `ASTRCODE_MAX_TOOL_CONCURRENCY_ENV` 和
/// `ASTRCODE_TOOL_RESULT_INLINE_LIMIT_ENV`，用于在不修改配置文件的情况下
/// 调整工具并发上限和全局工具结果落盘阈值。
...
/// 默认单个 agent 收件箱容量上限。
///
/// 超过此限制时 push_inbox 返回错误，防止消费端停滞时内存无限增长。
pub const DEFAULT_INBOX_CAPACITY: usize = 1024;

/// 默认单个会话的父级交付队列容量上限。
///
/// 超过此限制时 enqueue_parent_delivery 返回 false。
