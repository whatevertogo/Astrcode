    ...
    B: LauncherBackend,
{
    pub async fn resolve(
        ...
        let mut managed_server = self
            .backend
            .spawn_server(&SpawnRequest {
                working_dir: options.working_dir.clone(),
                server_binary: options.server_binary.clone(),
            })
            .await?;
...
async fn drain_stderr(stderr: ChildStderr) {
    ...
    while let Ok(Some(line)) = lines.next_line().await {
        eprintln!("[astrcode-server stderr] {line}");
    }
}
...
impl ManagedServerHandle for SystemManagedServer {
    async fn wait_ready(
        ...
        tokio::spawn(async move {
            while let Ok(Some(line)) = lines.next_line().await {
                eprintln!("[astrcode-server stdout] {line}");
            }
        });
...
mod tests {
    ...
    enum MockCall {
        ...
        Spawn(Result<MockManagedServer, LauncherError>),
    }
    ...
    async fn spawns_local_server_when_run_info_is_missing() {
        ...
        backend.push(MockCall::Spawn(Ok(MockManagedServer {
