...
impl PromptComposer {
    ...
    pub fn with_defaults() -> Self {
        Self::with_orchestration_config(OrchestrationConfig::default())
    }

    pub fn with_orchestration_config(orchestration_config: OrchestrationConfig) -> Self {
        Self::new(orchestration_config)
    ...
    fn lookup_cache(&self, contributor_id: &str, fingerprint: &str) -> Option<PromptContribution> {
        ...
        let ttl_valid = self.orchestration_config.cache_ttl.is_zero()
            || entry.cached_at.elapsed() <= self.orchestration_config.cache_ttl;
    ...
    fn effective_validation_level(&self, validation_policy: ValidationPolicy) -> ValidationLevel {
        match validation_policy {
            ValidationPolicy::Inherit => self.orchestration_config.validation_strictness,
    ...
    fn push_diagnostic(
        ...
        if !self.orchestration_config.enable_diagnostics {
            return;
        }
...
mod tests {
    ...
    async fn template_resolution_prefers_block_then_contributor_then_context_then_builtin() {
        ...
        let composer = PromptComposer::with_orchestration_config(OrchestrationConfig {
