...
///
/// 这里保留在 session crate，是因为它属于 session key 的 canonical 规则，
/// 不应该散落在 runtime façade 的 HTTP / service 入口里重复维护。
...
///
/// 返回 `AstrError::Validation` 或 `AstrError::io`，由上层按自己的协议语义再映射成
/// `ServiceError` / HTTP 状态码。
