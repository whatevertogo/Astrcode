...
/// runtime plugin 的共享摘要输入。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RuntimePluginSummary {
    pub name: String,
    pub version: String,
    pub description: String,
    pub state: astrcode_core::PluginState,
    pub health: astrcode_core::PluginHealth,
    pub failure_count: u32,
    pub failure: Option<String>,
    pub warnings: Vec<String>,
    pub last_checked_at: Option<String>,
    pub capabilities: Vec<RuntimeCapabilitySummary>,
}

/// 已解析的 runtime 状态摘要输入。
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ResolvedRuntimeStatusSummary {
    pub runtime_name: String,
    pub runtime_kind: String,
    pub loaded_session_count: usize,
    pub running_session_ids: Vec<String>,
...
/// 将治理快照解析为协议层可复用的摘要输入。
pub fn resolve_runtime_status_summary(
...
mod tests {
    use astrcode_core::{
        ...
        OperationMetricsSnapshot, PluginHealth, PluginManifest, PluginState, PluginType,
        ReplayMetricsSnapshot, SideEffect, Stability, SubRunExecutionMetricsSnapshot,
        plugin::PluginEntry,
    };
    use serde_json::json;

    use super::{GovernanceSnapshot, RuntimeObservabilitySnapshot, resolve_runtime_status_summary};
