...
fn sub_run_started_serializes_contract_fields_in_camel_case() {
    let payload = AgentEventPayload::SubRunStarted {
        ...
        agent: AgentContextDto {
            ...
            storage_mode: Some(SubRunStorageModeDto::IndependentSession),
    ...
    // descriptor 不再存在于 DTO 中
    assert!(data.get("descriptor").is_none());
}
...
#[test]
fn sub_run_finished_payload_roundtrip_without_descriptor() {
