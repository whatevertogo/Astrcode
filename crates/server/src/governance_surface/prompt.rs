...
pub fn build_delegation_metadata(
    description: &str,
    prompt: &str,
    _resolved_limits: &ResolvedExecutionLimitsSnapshot,
    _restricted: bool,
) -> astrcode_core::DelegationMetadata {
    let responsibility_summary = compact_delegation_summary(description, prompt);
    let reuse_scope_summary = "只有当下一步仍属于同一责任分支时，才应继续复用这个 \
                               child；若责任边界已经改变，应 close \
                               当前分支并重新选择更合适的执行主体。"
        .to_string();
    ...
    vec![governance_prompt_declaration(
        ...
        format!(
            "Use the child-agent tools as one decision protocol.\n\nKeep `agentId` exact. Copy it \
             ...
             task.\n\n`Idle` is normal and reusable. Do not respawn just because a child finished \
             one turn. Reuse an idle child with `send(agentId, message)` when the responsibility \
             stays the same. If you are unsure whether the child is still running, idle, or \
             terminated, call `observe(agentId)` once and act on the result.\n\nSpawn sparingly. \
             The runtime enforces a maximum child depth of {max_depth} and at most \
             {max_spawn_per_turn} new children per turn. Start with one child unless there are \
             clearly separate workstreams. Do not blanket-spawn agents just to explore a repo \
             broadly.\n\nAvoid waste:\n- Do not loop on `observe` with no decision attached.\n- \
             If a child is still running and you are simply waiting, prefer a brief shell sleep \
             over spending another tool call on `observe`.\n- Pick one wait mode per pause: \
             either `observe` now because you need a snapshot for the next decision, or sleep \
             briefly because you are only waiting. Do not alternate `shell` and `observe` in a \
             polling loop.\n- After a wait, call `observe` only when the next decision depends on \
             the child's current state.\n- Do not immediately re-`observe` the same child after a \
             fresh delivery unless the state is genuinely ambiguous.\n- Do not stack speculative \
             `send` calls.\n- Do not spawn a new child when an existing idle child already owns \
             the responsibility.\n\nIf a delivery satisfies the request, `close` the branch. If \
             the same child should continue, `send` one precise follow-up. If you see the same \
             `deliveryId` again after recovery, treat it as the same delivery, not a new \
             task.\n\nWhen you are the child on a delegated task, use upstream `send(kind + \
             payload)` to deliver a formal message to your direct parent. Report `progress`, \
             `completed`, `failed`, or `close_request` explicitly. Do not wait for the parent to \
             infer state from raw intermediate steps, and do not end with an open loop like \
             '继续观察中' unless you are also sending a non-terminal `progress` delivery that \
             keeps the branch alive.\n\nWhen you are the parent and receive a child delivery, \
             treat it as a decision point. Do not leave it hanging and do not immediately \
             re-observe the same child unless the state is unclear. Decide immediately whether \
             the result is complete enough to `close` the branch, or whether the same child \
             should continue with one concrete `send` follow-up that names the exact next step."
        ),
