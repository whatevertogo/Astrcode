...
struct ToolBridgeContext {
    ...
    session_storage_root: Option<std::path::PathBuf>,
    cancel: CancelToken,
...
impl ToolBridgeContext {
    fn from_tool_context(ctx: &ToolContext) -> Self {
        Self {
            ...
            session_storage_root: ctx.session_storage_root().map(|path| path.to_path_buf()),
    ...
    fn from_capability_context(ctx: &CapabilityContext) -> Self {
        Self {
            ...
            session_storage_root: ctx
                .metadata
                .get("serverSessionStorageRoot")
    ...
    fn into_capability_context(self, request_id: Option<String>) -> CapabilityContext {
        CapabilityContext {
            ...
            metadata: self.session_storage_root.as_ref().map(|path| serde_json::json!({ "serverSessionStorageRoot": path.to_string_lossy() })).unwrap_or(Value::Null),
    ...
    fn into_tool_context(self) -> ToolContext {
        ...
        if let Some(session_storage_root) = self.session_storage_root {
            tool_ctx = tool_ctx.with_session_storage_root(session_storage_root);
        }
