...
use astrcode_agent_runtime::{
    ...
    ToolDispatcher, TurnInput, TurnOutput, event_mapping::runtime_turn_events_to_agent_events,
};
use astrcode_context_window::tool_result_budget::ToolResultReplacementRecord;
use astrcode_core::{
    AgentInboxEnvelope, AgentLifecycleStatus, AgentTurnOutcome, AstrError, CancelToken,
...
impl ToolDispatcher for RouterToolDispatcher {
    async fn dispatch_tool(
        ...
        if let Some(session_storage_root) = &self.session_storage_root {
            tool_ctx = tool_ctx.with_session_storage_root(session_storage_root.clone());
        }
...
impl SessionRuntimeCompatPort {
    ...
    fn spawn_turn_execution(&self, input: SpawnTurnExecutionInput) {
        ...
        tokio::spawn(async move {
            ...
            let session_storage_root = astrcode_support::hostpaths::project_dir(&working_dir).ok();
            let events_history_path = session_storage_root.as_ref().map(|project_dir| {
                project_dir
                    .join("sessions")
...
impl SessionRuntimePort for SessionRuntimeCompatPort {
    ...
    async fn observe_agent_session(
        ...
        Ok(SessionObserveSnapshot::from_projected_state(
            lifecycle_status,
            &projected,
    ...
    let bridge = tokio::spawn(async move {
        ...
        while let Some(event) = receiver.recv().await {
            ...
            if let Some(state) = &session_state {
                for agent_event in runtime_turn_events_to_agent_events(&event, &agent) {
...
mod tests {
    ...
    use super::{
        ...
        runtime_turn_events_to_agent_events,
    };
    use crate::{
    ...
    async fn router_tool_dispatcher_updates_mode_contract_after_enter_plan_mode() {
        ...
        let dispatcher = RouterToolDispatcher {
            ...
            session_storage_root: Some(temp.path().join("projects")),
            cancel: CancelToken::new(),
            agent: AgentEventContext::root_execution("agent-root", "default"),
            mode_tool_state: mode_tool_state.clone(),
            event_sink: Arc::new(RuntimeToolEventSink {
