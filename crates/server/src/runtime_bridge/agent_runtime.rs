...
use crate::{
    AgentOrchestrationService, GovernanceSurfaceAssembler, ProfileProvider,
...
impl ProfileProvider for ServerProfileProviderAdapter {
    fn load_for_working_dir(
        &self,
        working_dir: &Path,
    ) -> Result<Vec<astrcode_core::AgentProfile>, ServerApplicationError> {
        self.profiles
            .resolve(working_dir)
    ...
    fn load_global(&self) -> Result<Vec<astrcode_core::AgentProfile>, ServerApplicationError> {
        self.profiles
            .resolve_global()
            .map(|profiles| profiles.as_ref().clone())
            .map_err(server_route_error_to_application_error)
...
fn server_route_error_to_application_error(error: ServerRouteError) -> ServerApplicationError {
    match error {
