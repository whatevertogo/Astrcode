...
pub async fn launch_subagent(
...
) -> Result<LaunchedSubagent, ServerApplicationError> {
    ...
    let handle =
        persist_resolved_limits_for_handle(kernel, handle, surface.resolved_limits.clone())
            .await
            .map_err(ServerApplicationError::Internal)?;
...
fn ensure_subagent_profile_mode(profile: &AgentProfile) -> Result<(), ServerApplicationError> {
...
fn map_spawn_error(error: ServerKernelControlError) -> ServerApplicationError {
    match error {
        ServerKernelControlError::MaxDepthExceeded { current, max } => {
        ...
        ServerKernelControlError::MaxConcurrentExceeded { current, max } => {
            ServerApplicationError::Conflict(format!(
                "too many active agents ({current}/{max}); wait for an existing child to go idle \
                 or close one before spawning more"
            ))
        },
        ServerKernelControlError::ParentAgentNotFound { agent_id } => {
            ServerApplicationError::NotFound(format!("parent agent '{agent_id}' not found"))
        },
    }
}
...
mod tests {
    ...
    fn map_spawn_error_turns_depth_limit_into_actionable_invalid_argument() {
        let err =
            map_spawn_error(ServerKernelControlError::MaxDepthExceeded { current: 4, max: 3 });

        assert!(matches!(err, ServerApplicationError::InvalidArgument(_)));
    ...
    fn map_spawn_error_turns_concurrency_limit_into_conflict() {
        let err =
            map_spawn_error(ServerKernelControlError::MaxConcurrentExceeded { current: 8, max: 4 });

        assert!(matches!(err, ServerApplicationError::Conflict(_)));
