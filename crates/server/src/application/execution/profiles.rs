...
use crate::errors::ServerApplicationError;
...
pub trait ProfileProvider: Send + Sync {
    ...
    fn load_for_working_dir(
        &self,
        working_dir: &Path,
    ) -> Result<Vec<AgentProfile>, ServerApplicationError>;

    /// 加载不绑定项目 scope 的全局 agents。
    fn load_global(&self) -> Result<Vec<AgentProfile>, ServerApplicationError>;
}
...
impl ProfileResolutionService {
    ...
    pub fn resolve(
        &self,
        working_dir: &Path,
    ) -> Result<Arc<Vec<AgentProfile>>, ServerApplicationError> {
    ...
    pub fn resolve_global(&self) -> Result<Arc<Vec<AgentProfile>>, ServerApplicationError> {
        {
    ...
    pub fn find_profile(
        ...
        working_dir: &Path,
        profile_id: &str,
    ) -> Result<AgentProfile, ServerApplicationError> {
        ...
        profiles
            ...
            .cloned()
            .ok_or_else(|| {
                ServerApplicationError::NotFound(format!(
...
mod tests {
    ...
    fn find_profile_returns_not_found_when_missing() {
        ...
        assert!(
            matches!(err, ServerApplicationError::NotFound(ref msg) if msg.contains("nonexistent")),
    ...
    fn find_profile_returns_not_found_even_with_cache_hit() {
        ...
        assert!(
            matches!(err, ServerApplicationError::NotFound(_)),
            "缓存命中不影响业务校验: {err}"
        );
    }
    ...
    fn find_global_profile_returns_not_found_when_missing() {
        ...
        assert!(
            matches!(err, ServerApplicationError::NotFound(ref msg) if msg.contains("nobody")),
            "should be NotFound: {err}"
        );
    }
