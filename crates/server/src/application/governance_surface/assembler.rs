...
use crate::{
    AgentSessionPort, CompiledModeEnvelope, ExecutionControl, ServerApplicationError,
...
impl GovernanceSurfaceAssembler {
    ...
    ) -> Result<CompiledModeEnvelope, ServerApplicationError> {
        let spec = self.mode_catalog.get(mode_id).ok_or_else(|| {
            ServerApplicationError::InvalidArgument(format!("unknown mode '{}'", mode_id))
        })?;
        compile_mode_envelope(&spec, extra_prompt_declarations)
    ...
    fn compile_child_mode_surface(
        ...
        compile_mode_envelope_for_child(&spec).map_err(ServerApplicationError::from)
    }

    fn build_surface(
        &self,
        input: BuildSurfaceInput,
    ) -> Result<ResolvedGovernanceSurface, ServerApplicationError> {
