...
use astrcode_core::{
    ...
    ResolvedExecutionLimitsSnapshot, Result, ToolContext, mode::ModeId,
};
...
fn default_resolved_limits() -> ResolvedExecutionLimitsSnapshot {
...
async fn ensure_handle_has_resolved_limits(
    ...
    super::persist_resolved_limits_for_handle(kernel, handle, default_resolved_limits()).await
}
...
impl AgentOrchestrationService {
    ...
    pub(super) fn resolve_subagent_profile(
        ...
        self.profiles
            ...
            .map_err(|error| match error {
                crate::ServerApplicationError::NotFound(message) => {
                    AgentOrchestrationError::NotFound(message)
                },
                crate::ServerApplicationError::InvalidArgument(message) => {
    ...
    pub(super) async fn ensure_parent_agent_handle(
        ...
        if let Some(agent_id) = explicit_agent_id {
            if let Some(handle) = self.kernel.get_handle(&agent_id).await {
                if handle.depth == 0 && handle.resolved_limits != ResolvedExecutionLimitsSnapshot {
                    return ensure_handle_has_resolved_limits(self.kernel.as_ref(), handle)
                        .await
        ...
        ensure_handle_has_resolved_limits(self.kernel.as_ref(), handle)
            .await
            .map_err(AgentOrchestrationError::Internal)
    }
