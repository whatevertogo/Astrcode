...
use crate::{
    ApiError, AppState, auth::require_auth, composer_catalog::list_session_composer_options,
    mapper::to_composer_options_response, routes::sessions::validate_session_path_id,
};
...
pub(crate) async fn session_composer_options(
    ...
    let items = list_session_composer_options(
        &state,
        &session_id,
        query.q.as_deref(),
        &requested_kinds,
