...
use astrcode_core::{
    AgentEventContext, AstrError, LlmMessage, PersistedToolOutput, Result, StorageEvent,
    StorageEventPayload,
...
pub(crate) struct ToolResultBudgetStats {
...
#[derive(Debug, Clone)]
pub(crate) struct ToolResultBudgetOutcome {
    pub messages: Vec<LlmMessage>,
    pub events: Vec<StorageEvent>,
    pub stats: ToolResultBudgetStats,
}

pub(crate) struct ApplyToolResultBudgetRequest<'a> {
...
pub(crate) fn project_dir(working_dir: &Path) -> Result<PathBuf> {
...
fn resolve_home_dir() -> Result<PathBuf> {
    ...
    Err(AstrError::HomeDirectoryNotFound)
}
