...
mod tests {
    use astrcode_core::{
        AgentEventContext, CompactAppliedMeta, CompactMode, CompactTrigger, PersistedToolOutput,
        StorageEvent, StorageEventPayload, SubRunStorageMode, TurnTerminalKind,
    };
    use serde_json::json;
    ...
    fn turn_done_sets_idle_and_increments_count() {
        let events = vec![
            ...
            turn_done(None, root_agent(), TurnTerminalKind::Completed),
        ];
        let state = project(&events);
        assert_eq!(state.phase, Phase::Idle);
    ...
    fn sub_run_events_do_not_pollute_parent_projected_messages_or_turn_count() {
        let events = vec![
            ...
            turn_done(Some("turn-root"), root_agent(), TurnTerminalKind::Completed),
            user_message(
                Some("turn-child"),
    ...
    fn independent_sub_run_events_still_project_into_child_session_state() {
        ...
        let events = vec![
            ...
            turn_done(Some("turn-child"), child_agent, TurnTerminalKind::Completed),
        ];
    ...
    fn compact_applied_replaces_old_prefix_with_a_compact_summary_message() {
        let events = vec![
            ...
            turn_done(Some("turn-1"), root_agent(), TurnTerminalKind::Completed),
            user_message(
                Some("turn-2"),
                root_agent(),
                "second",
            ...
            turn_done(Some("turn-2"), root_agent(), TurnTerminalKind::Completed),
