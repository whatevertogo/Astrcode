...
use astrcode_core::{CapabilityContext, CapabilityExecutionResult, CapabilityInvoker, Result};
...
impl CapabilityInvoker for McpPromptBridge {
    fn descriptor(&self) -> CapabilityDescriptor {
        CapabilityDescriptor::builder(&self.fully_qualified_name, CapabilityKind::tool())
            ...
            .tag("mcp:prompt")
            .build()
            .expect("mcp prompt descriptor must build")
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn builds_prompt_descriptor_from_arguments() {
        ...
        assert_eq!(descriptor.name, "mcp__srv__review");
        assert!(descriptor.tags.iter().any(|tag| tag == "mcp:prompt"));
    }
