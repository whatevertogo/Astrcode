...
use astrcode_core::{
    CapabilityContext, CapabilityExecutionResult, CapabilityInvoker, Result,
...
impl CapabilityInvoker for ListMcpResourcesTool {
    fn descriptor(&self) -> CapabilityDescriptor {
        CapabilityDescriptor::builder("mcp_list_resources", CapabilityKind::tool())
...
impl CapabilityInvoker for ReadMcpResourceTool {
    fn descriptor(&self) -> CapabilityDescriptor {
        CapabilityDescriptor::builder("mcp_read_resource", CapabilityKind::tool())
...
mod tests {
    ...
    #[test]
    fn test_list_resources_descriptor() {
        let conns = Arc::new(Mutex::new(HashMap::new()));
        let tool = ListMcpResourcesTool::new(conns);
        let desc = tool.descriptor();
        assert_eq!(desc.name, "mcp_list_resources");
    }

    #[test]
    fn test_read_resource_descriptor() {
        let conns = Arc::new(Mutex::new(HashMap::new()));
        let tool = ReadMcpResourceTool::new(conns);
        let desc = tool.descriptor();
        assert_eq!(desc.name, "mcp_read_resource");
    }
}
