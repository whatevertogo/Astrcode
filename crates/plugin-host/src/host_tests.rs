...
#[tokio::test]
async fn reload_with_external_backends_rolls_back_candidate_on_backend_failure() {
    let root = unique_temp_dir("reload-rollback");
    fs::create_dir_all(&root).expect("temp dir should create");
    fs::write(
...
"#,
    ...
    for backend in &mut first.external_backends {
        backend.shutdown().await.expect("shutdown should succeed");
    }
...
plugin_type = ["Tool"]
capabilities = []
executable = "definitely-missing-command.exe"
args = []
working_dir = "."
...
"#,
    ...
    let error = host
        ...
        .await
        .expect_err("reload should fail when backend start fails");

    assert!(
        error
            .to_string()
            .contains("failed to spawn plugin backend 'beta'")
    );
    assert!(host.registry().candidate_snapshot().is_none());
    assert_eq!(
        host.active_snapshot()
            .expect("previous active snapshot should remain")
            .plugin_ids,
        vec!["alpha".to_string()]
    );
