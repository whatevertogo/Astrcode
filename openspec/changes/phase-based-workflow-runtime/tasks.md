...
- [ ] 2.5 引入 per-turn 具体 `TurnCoordinator`，把 `accept → prepare → run → persist → finalize → deferred_compact` 收为单一协调器的显式生命周期方法；`submit.rs` 简化为请求解析 + `TurnCoordinator::start()`；`execution.rs` helper 合并入 `TurnRuntimeState` 或 `TurnCoordinator`；`interrupt_session()` 和 `fork_session()` 走 `TurnCoordinator`；验证：`cargo test -p astrcode-session-runtime --lib`
...
- [ ] 2.7 引入 `PostLlmDecisionPolicy`，合并 `continuation_cycle.rs`、`loop_control.rs::decide_budget_continuation`、`step/mod.rs` 中“无工具输出后下一步”的散落逻辑；step 主循环变成 `match policy.decide()` 的决策表；验证：新增 policy 单元测试并运行 `cargo test -p astrcode-session-runtime --lib`
