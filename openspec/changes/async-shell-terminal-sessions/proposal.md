...
当前 `Astrcode` 的工具调用仍以同步 turn 执行为主：长时间运行的 shell / tool 会直接阻塞 turn loop，而现有运行时又没有一套像 Claude Code 那样的后台任务注册、输出落盘与完成通知机制。同时，现有 `shell` 只覆盖一次性非交互命令，无法提供持久终端会话、stdin 回写与显式读取输出语义，导致 LLM 无法像现代 coding agent 那样稳定操控终端。
...
- 保留现有 `shell` 作为一次性命令工具，并增加 `background` 执行模式；同时新增持久终端会话工具族，使 LLM 可以启动终端、写入 stdin、显式读取新输出并关闭会话。
...
### New Capabilities
- `async-tool-execution`: 定义工具如何立即返回后台任务句柄、持续产出输出、发送完成通知并暴露显式读取语义。
...
- 用户可见影响：
  ...
  - LLM 可以通过正式工具直接操控持久终端会话
- 开发者可见影响：
  - 工具执行从“同步完成即返回”演进为“foreground / background / terminal-session”多模式合同
