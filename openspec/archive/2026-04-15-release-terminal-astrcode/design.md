...
### 决策 4：conversation surface 从第一天起使用单一版本轴

新增 conversation surface 不沿用“先上 unversioned 路径，后续再补版本”的策略，而是直接采用 path version：

- snapshot：`GET /api/v1/conversation/sessions/{id}/snapshot`
- stream：`GET /api/v1/conversation/sessions/{id}/stream?cursor=...`
- `protocol` 中新增对应版本命名空间，例如 `http::conversation::v1`
...
本次变更不强行给全仓库所有旧 `/api/*` 路由补齐统一版本迁移，但 conversation surface 必须从起点就可演进。

conversation v1 不再叠加第二套 surface 内部版本号：
...
是否在本次顺带引入 `schemars` / TypeScript 导出，不作为硬前置；如果 conversation surface 很快要被多语言 client 消费，再单独补 schema export。
...
### 决策 6：conversation v1 采用“snapshot + SSE stream”合同，不复用旧 `/events`

conversation v1 的 authoritative read surface 明确为：
...
- [Risk] 新增 conversation read model 后，GUI 与 TUI 可能再次出现两套近似但不完全一致的视图语义  
  Mitigation：把 conversation 投影定义为正式 contract，并把 authoritative builder 固定在服务端边界侧；不要在任一 client 侧复制 reducer。
...
- `protocol`
  - 为 conversation v1 DTO、cursor、错误 envelope、新增 delta kind 增加 fixture + conformance tests
- `client`
...
4. 在 `application` 中补齐 `ConversationFacts` 查询编排；在服务端边界侧落地 `conversation_projection` 与 `/api/v1/conversation/*` authoritative route。
...
- 若 conversation surface 路由已合入但客户端未发布，可保留未被主文档暴露的路由，不影响现有 GUI 合同
...
- conversation v1 是否在本次 change 就顺带产出 schema export，还是先用 protocol fixture/conformance 冻结 JSON 形状？
