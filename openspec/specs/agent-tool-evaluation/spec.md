...
系统 MUST 基于原始协作事实生成稳定的诊断读模型 `AgentCollaborationScorecardSnapshot`，用于判断 agent-tool 是否创造了实际协作价值。此外，协作评估的原始事实 MUST 可被评测 trace 提取器作为输入源，在评测场景中建立与 collaboration facts 的关联。
