...
定义 governance mode 的核心系统架构，包括 mode catalog 管理、envelope 编译、事件持久化、mode transition 校验、child policy 推导与观测性。
...
#### Scenario: builtin code mode is available by default
...
- **AND** plan mode 的 selector 排除 `SideEffect::Local`、`SideEffect::Workspace`、`SideEffect::External` 和 `Tag("agent")` 的工具
- **AND** 额外保留 `exitPlanMode` 和 `upsertSessionPlan` 两个工具
...
- **THEN** replay 结果 SHALL 回退到 builtin `code` mode
...
- **THEN** `code`、`plan`、`review` 等 builtin mode SHALL 已在 catalog 中注册
- **AND** 无需任何额外配置即可使用
