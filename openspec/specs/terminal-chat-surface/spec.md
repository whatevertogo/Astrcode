...
终端客户端 MUST 在有限终端空间内稳定展示 thinking、tool call、stdout / stderr streaming 与 child agent / subagent 活动；主 transcript 与 child 细节视图必须共享同一服务端事实，但终端布局不得要求用户阅读原始低层事件流。
